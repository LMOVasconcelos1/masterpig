...
<div class="max-w-7xl mx-auto space-y-6 pb-10">
    <div class="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
            <div class="text-xs font-bold text-gray-600 uppercase tracking-wider">Plantel</div>
            <h1 class="text-2xl font-bold text-gray-900">Cadastro de Fêmeas</h1>
            <div class="text-sm text-gray-500 mt-1">Listagem geral com status e última movimentação.</div>
        </div>
        <div class="flex items-center gap-3">
            <a href="{{ route('dashboard', [], false) }}" class="inline-flex items-center rounded-xl border border-gray-200 shadow-sm px-4 py-2 bg-white text-sm font-semibold text-gray-700 hover:bg-gray-50">
    ...
    <div class="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden" x-data="{ q: '' }">
        <div class="p-6 border-b border-gray-100 bg-gray-50/50 flex flex-col gap-3 md:flex-row md:items-center md:justify-between">
            <div class="w-full md:max-w-md">
                <div class="relative">
                    <i class="fa-solid fa-magnifying-glass absolute left-4 top-1/2 -translate-y-1/2 text-gray-400"></i>
                    <input type="text" x-model="q" placeholder="Buscar por ID, localização, tipo..." class="w-full pl-11 rounded-xl border-gray-200 shadow-sm focus:ring-primary-500 focus:border-primary-500 text-sm">
                </div>
            </div>

            <div class="flex items-center gap-3">
                <a href="{{ route('admin.plantel.femeas.index', [], false) }}" class="inline-flex items-center px-3 py-2 rounded-xl border border-gray-200 bg-white text-sm font-semibold {{ !$mostrarInativas ? 'text-primary-700 border-primary-200 bg-primary-50' : 'text-gray-700' }}">
                    Ativas
                </a>
                <a href="{{ route('admin.plantel.femeas.index', ['inativas' => 1], false) }}" class="inline-flex items-center px-3 py-2 rounded-xl border border-gray-200 bg-white text-sm font-semibold {{ $mostrarInativas ? 'text-primary-700 border-primary-200 bg-primary-50' : 'text-gray-700' }}">
                    Inativas
                </a>
